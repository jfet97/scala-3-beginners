...
object AccessModifiers {
  ...
  class Person(val name: String) {
    ...
    private def watchNetflix(): String = "No, I hate TV series..." // only accessible inside the class
  }
  ...
  // complication
  class KidWithParents(name: String, age: Int, momName: String, dadName: String) extends Person(name) {
    val mom = new Person(momName)
    val dad = new Person(dadName)

    def everyoneSayHi(): String =
      this.sayHi() + " Hi, I'm $name, and here are my parents" // + mom.sayHi() + dad.sayHi() X
  }
